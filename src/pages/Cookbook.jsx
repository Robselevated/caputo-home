                ...
                {ingredients.map((ing, i) => (
                  <div key={i} className="space-y-2 pb-2 border-b border-warmgray-100 last:border-0">
                    <div className="flex gap-2 items-start">
                      <input type="text" value={ing.name} onChange={(e) => updateIngredient(i, 'name', e.target.value)} placeholder="Ingredient name" className="input-field focus:ring-section-cookbook flex-1" />
                      {ingredients.length > 1 && (
                        <button type="button" onClick={() => removeIngredient(i)} className="text-red-500 hover:text-red-600 mt-3 shrink-0">
                          <span className="material-symbols-outlined text-xl">close</span>
                        </button>
                      )}
                    </div>
                    <div className="flex gap-2">
                      <input type="text" value={ing.qty} onChange={(e) => updateIngredient(i, 'qty', e.target.value)} placeholder="Qty" className="input-field focus:ring-section-cookbook w-20" inputMode="decimal" />
                      <input type="text" value={ing.unit} onChange={(e) => updateIngredient(i, 'unit', e.target.value)} placeholder="Unit (cup, tbsp...)" className="input-field focus:ring-section-cookbook flex-1" />
                    </div>
                  </div>
                ))}
                <button type="button" onClick={addIngredientRow} className="text-sm text-section-cookbook font-medium">
                  + Add Ingredient
                </button>
              </div>
