  ...
  return (
    <div className="mb-3">
      <button
        ...
        className={`w-full flex items-center justify-between px-4 py-3 ${colorClass} bg-opacity-10 rounded-xl`}
      >
...
function InventoryItem({ item, onUpdateQty, onDelete, onAddToGrocery, userId }) {
  ...
  return (
    <div className={`flex items-center gap-2 px-4 py-3 border-b border-gray-50 last:border-0 ${item.qty === 0 ? 'bg-red-50/50' : ''}`}>
      <div className="flex-1 min-w-0">
        <p className="font-medium text-gray-900 text-sm">
          {item.name}
          {item.qty === 0 && (
            <span className="ml-2 text-[10px] bg-red-100 text-red-600 px-1.5 py-0.5 rounded font-medium">OUT</span>
          )}
        </p>
      ...
      <div className="flex items-center gap-1 shrink-0">
        <button
          ...
          className="w-7 h-7 rounded-lg bg-gray-100 flex items-center justify-center text-gray-600 active:bg-gray-200"
        >
        ...
        {editing ? (
          <input
            ...
            className="w-12 text-center text-sm font-semibold border border-gray-300 rounded-lg py-0.5"
            autoFocus
            inputMode="decimal"
          />
        ) : (
          ...
          >
            {item.qty}
            <span className="text-[10px] text-gray-400 block leading-none">{item.unit}</span>
          </button>
        )}
