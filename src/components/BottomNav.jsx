...
export default function BottomNav() {
  return (
    <nav className="fixed bottom-0 left-0 w-full z-50 flex justify-around items-center h-20 bg-white/70 backdrop-blur-xl rounded-t-[2rem] px-4 pb-safe shadow-[0_-1px_0_rgba(0,0,0,0.04),0_-12px_32px_rgba(44,47,48,0.06)]">
      {navItems.map((item) => (
        <NavLink
          key={item.path}
          to={item.path}
          className={({ isActive }) =>
            `flex flex-col items-center justify-center p-2 transition-transform duration-200 ${
              isActive
